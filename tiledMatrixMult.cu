...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
  ...
  //@@ You have to use shared memory for this MP
	...
	float CValue = 0.0;
	for( int tIdx = 0; tIdx < (numBRows-1)/TILE_WIDTH + 1; ++tIdx )
	{
		if( Row < numARows && tIdx*TILE_WIDTH+threadIdx.x < numAColumns)
			tileA[threadIdx.y][threadIdx.x] = A[Row*numAColumns + tIdx*TILE_WIDTH + threadIdx.x];
		else
			tileA[threadIdx.y][threadIdx.x] = 0.0;
		
		if( tIdx*TILE_WIDTH+threadIdx.y < numBRows && Col < numBColumns) 
			tileB[threadIdx.y][threadIdx.x] = B[(tIdx*TILE_WIDTH+threadIdx.y)*numBColumns + Col];
		else
...
int main(int argc, char **argv) {
  ...
  hostC =
      ( float * )malloc(sizeof(float)*numCRows*numCColumns);
